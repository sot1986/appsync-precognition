...
import { datetime, baseErrors as errors, integer } from '../src/utils'
...
describe.concurrent('test dateRule validation', () => {
  it.concurrent.each([
    ...
    const result = rules.parse({ value, errors }, ['regex', datetime])
  ...
  it.concurrent.each([
    ...
    const result = rules.parse({ value: String(value), errors }, ['regex', datetime, integer])
    expect(result.check).toBe(true)
  })
})
